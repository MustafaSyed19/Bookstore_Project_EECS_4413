...
import { cartApi, userApi, productApi } from '../api/api';
...
function CheckoutForm() {
  ...
  const handleSubmit = async () => {
    ...
    for (const item of cart.items) {
      try {
        ...
        if (product.quantity < item.quantity) {
          setInventoryError(
            `"${item.title}" only has ${product.quantity} in stock, but you requested ${item.quantity}. Please update your cart.`
          );
          setSubmitting(false);
          return;
        }
      } catch (err) {
        toast(err.message || 'Failed to validate inventory', 'error');
        setSubmitting(false);
        return;
      }
    }
    ...
    try {
      ...
      // IMPORTANT:
      // Only send what the backend route expects.
      // userId comes from verifyToken on the backend.
      const orderData = {
        items: cart.items.map(item => ({
          bookId: item.bookId,
          quantity: item.quantity,
        })),
        shippingAddressId: useSavedAddress && savedAddress ? savedAddress.id : null,
      };

      // Create order ONCE through AppContext
      ...
      // Show a useful success object for UI rendering
      setOrderResult({
        id: createdOrder.orderId ?? createdOrder.id ?? `ORD-${Date.now()}`,
        total: createdOrder.totalAmount ?? cart.total,
      ...
      <div className="container" style={{ paddingTop: 40, paddingBottom: 60, maxWidth: 700, margin: '0 auto' }}>
        <div className="checkout-result fade-up">
          ...
          <div className="profile-section">
            ...
            >
              <span>Total</span>
              <span style={{ color: 'var(--rust)' }}>${Number(orderResult.total).toFixed(2)}</span>
            </div>
          </div>
      ...
      <div className="container" style={{ paddingTop: 40, paddingBottom: 60, maxWidth: 700, margin: '0 auto' }}>
        <div className="checkout-result fade-up">
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button className="btn btn-primary" onClick={() => { setStep('form'); setErrors({}); }}>
              Try Again
            </button>
      ...
      <div className="checkout-grid">
              ...
              <>
                <div className="input-group">
                  ...
                  <input
                    ...
                    onChange={e => setShipField('street', e.target.value)}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div className="input-group">
                    ...
                    <input
                      ...
                      onChange={e => setShipField('province', e.target.value)}
                    />
                ...
                <div className="input-group">
                  ...
                  <input
                    ...
                    onChange={e => setShipField('phone', e.target.value)}
                  />
                </div>
              </>
            )}
                ...
                <div className="input-group">
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setCardField('cardNumber', v.replace(/(\d{4})(?=\d)/g, '$1 '));
                    }}
        ...
        <div className="cart-summary fade-up" style={{ animationDelay: '0.15s' }}>
            ...
            >
              {item.imageUrl ? (
                <img
                  src={item.imageUrl}
                  alt={item.title}
                  style={{ width: 50, height: 65, objectFit: 'cover', borderRadius: 4, background: 'var(--warm)', flexShrink: 0 }}
          ...
          <div className="summary-row" style={{ marginBottom: 24 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 22, color: 'var(--rust)' }}>
